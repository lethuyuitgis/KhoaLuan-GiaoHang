        ...
        <div className="px-4 py-6 bg-white space-y-3">
          {import.meta.env.DEV && (
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-3 text-xs text-amber-800 flex items-start gap-2">
              <span>⚠️</span>
              <span>
                Đang chạy ngoài Telegram (dev mode). Một số API có thể không hoạt động —
                hãy mở Mini App qua bot Telegram để có đầy đủ tính năng.
              </span>
            </div>
          )}
          <button
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center px-6">
        ...
        <h1 className="text-2xl font-bold text-orange-600">Shop Giao Hàng</h1>
        <div className="mt-6 inline-block w-8 h-8 border-2 border-orange-200 border-t-orange-500 rounded-full animate-spin"></div>
        <p className="mt-4 text-sm text-gray-500">Đang xác thực…</p>
      </div>
    );
  }
