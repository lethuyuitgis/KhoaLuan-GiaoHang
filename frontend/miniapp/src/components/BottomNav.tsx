...
export function BottomNav() {
  ...
  // Hide on shipper routes — they have their own UX.
  if (pathname.startsWith('/shipper')) return null;
  // Hide on splash so the brand intro feels full-bleed.
  if (pathname === '/' || pathname === '') return null;

  const tabs: TabDef[] = [
    { to: '/customer/shop',   label: t('nav.home'),   match: (p: string) => p === '/customer/shop',                                Icon: IconHome },
    { to: '/customer/shop',   label: t('nav.menu'),   match: (p: string) => p.startsWith('/customer/shop'),                        Icon: IconMenu, hidden: true },
    { to: '/customer/cart',   label: t('nav.cart'),   match: (p: string) => p.startsWith('/customer/cart') || p.startsWith('/customer/checkout'), Icon: IconCart, badge: itemCount },
    { to: '/customer/orders', label: t('nav.orders'), match: (p: string) => p.startsWith('/customer/orders'),                      Icon: IconOrders },
